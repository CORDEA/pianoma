...
export class NotesGenerator {
  ...
  generate(): Question {
    ...
    const bass =
      bassConcurrency === 0 ? null : this.generateNotes(bassConcurrency, false);
    return {
  ...
  private generateNotes(
    ...
    const numberOfNotes = isHalf
      ? Math.floor(this.maxNotes / 2)
      : this.maxNotes;
    const notes = Array.from(Array(numberOfNotes).keys()).map((i) => {
      const concurrency = Math.floor(Math.random() * maxConcurrency) + 1;
      return {
        concurrentNotes: Array.from(Array(concurrency).keys()).map((_) =>
          NotesGenerator.generateNote(isTreble)
        ),
        tempo: tempo,
      };
    });
