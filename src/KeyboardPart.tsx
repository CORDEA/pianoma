...
function KeyboardPart(props: Props) {
  ...
  const whiteKeys = Array.from(Array(props.numberOfBlackKeys + 1).keys()).map(
    (i) => {
      const note = NOTES[noteIndex + i] + props.startLevel;
      return (
        <KeyboardWhiteKey note={note} key={note} strokeColor={strokeColor} />
      );
    }
  );
  const blackKeys = Array.from(Array(props.numberOfBlackKeys).keys()).map(
    (i) => {
      const note = NOTES[noteIndex + i] + "#" + props.startLevel;
  ...
  return (
    <div className="KeyboardPart" style={{ width: width }}>
      <div className="KeyboardPart-whiteKeys">{whiteKeys}</div>
      <div className="KeyboardPart-blackKeys">{blackKeys}</div>
    </div>
  );
}
